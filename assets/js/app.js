...
console.log('ProteoERP Common Utilities Loaded');
    ...
    const initApp = async () => {
        ...
        const checkAuth = async () => {
            const apiPath = ROOT_PATH + 'api.php';
            
            try {
                const resp = await fetch(`${apiPath}?action=me`);
                const user = await resp.json();
                
                if (!user || (!user.logged_in && !isLoginPage)) {
                    window.location.href = (window.location.pathname.includes('vistas/')) ? (window.location.pathname.includes('reportes/') ? '../../index.php' : '../index.php') : 'index.php';
                    return null;
                }
                return user;
            } catch (e) {
        ...
        function setupNavbar(user) {
            ...
            const logoutUrl = ROOT_PATH + 'logout.php';
