...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'vendedores') {
    ...
    if (!empty($f_txt) && strlen(trim($f_txt)) >= 2) {
        ...
        $where .= " AND (i.desca LIKE :txt OR i.codigoa LIKE :txt OR f.usuario LIKE :txt)";
    ...
    try {
        $stmt = $pdo->prepare("
            SELECT 
                ...
                COUNT(DISTINCT i.codigoa) as total_articulos,
                SUM(i.cana) as cantidad_total,
                SUM(i.totad) as monto_total
            FROM itpfac i
...
// --- Manejo AJAX: Filtrar por Televendedor ---
if (isset($_GET['ajax']) && $_GET['ajax'] === 'filtrar') {
    ...
    try {
        $params = [':ini' => $f_ini, ':fin' => $f_fin];
        $where = " AND f.fecha >= :ini AND f.fecha <= :fin ";
        
        if (!empty($codvend)) {
            $where .= " AND f.usuario = :codvend ";
        ...
        $stmt = $pdo->prepare("
            SELECT 
                ...
                COALESCE(NULLIF(f.usuario, ''), 'Sin Televendedor') as nombre_vendedor,
                i.cana as cantidad_actual,
                ROUND(i.preca, 2) as precio,
                ROUND(i.precad, 2) as precio_d,
                ROUND(i.tota, 2) as total,
                ROUND(i.totad, 2) as total_d,
                ROUND(i.iva, 2) as iva_bs
            FROM itpfac i
            INNER JOIN pfac f ON i.numa = f.numero
            WHERE 1=1
            $where
            ORDER BY f.fecha DESC, i.codigoa
        ");
        $stmt->execute($params);
...
try {
    // Lista de televendedores para filtro
    $stmt_vend = $pdo->query("
        ...
        ORDER BY usuario
    ");
    $vendedores_filter = $stmt_vend->fetchAll(PDO::FETCH_ASSOC);

    // --- Query Principal: Listado de Artículos Vendidos ---
    $params = [':ini' => $f_ini, ':fin' => $f_fin];
    $where = "WHERE f.fecha >= :ini AND f.fecha <= :fin";
    ...
    // Resumen por Televendedor
    $stmt_res = $pdo->prepare("
        SELECT 
            f.usuario as codvend,
            COALESCE(NULLIF(f.usuario, ''), 'Sin Televendedor') as vendedor,
            COUNT(DISTINCT i.codigoa) as articulos,
            SUM(i.cana) as cantidad,
            SUM(i.totad) as monto,
    ...
    $resumen_vend = $stmt_res->fetchAll(PDO::FETCH_ASSOC);
    ...
    $stmt_det = $pdo->prepare("
        ...
        INNER JOIN pfac f ON i.numa = f.numero
        $where
        ORDER BY f.fecha DESC, i.codigoa DESC
    ...
    $stmt_count = $pdo->prepare("
        SELECT COUNT(DISTINCT i.codigoa) as total
        FROM itpfac i
        INNER JOIN pfac f ON i.numa = f.numero
        $where
    ");
...
<div class="content-wrapper">
    ...
    <nav class="module-nav">
        ...
        <a href="televentas_articulos.php" class="nav-item active">
    ...
    <div class="page-title">
        <h1><i class="fas fa-shopping-cart" style="color:var(--primary);"></i> Listado de Artículos Vendidos (Televentas)</h1>
        <p>Reporte consolidado de ventas por vendedor y producto &bull; 
    ...
    <div class="metrics-grid">
        ...
        <div class="card metric-card primary">
            ...
            <div class="metric-content">
                <span class="metric-label">Cantidad Total Vendida</span>
        ...
        <div class="card metric-card success">
            ...
            <div class="metric-content">
                <span class="metric-label">Ventas Total (USD)</span>
                <p class="metric-value"><?php echo formatUSD($kpis['total_usd'] ?? 0); ?></p>
            </div>
        </div>
        <div class="card metric-card info">
            <div class="metric-icon"><i class="fas fa-user-tie"></i></div>
            <div class="metric-content">
                <span class="metric-label">Vendedores Activos</span>
                <p class="metric-value"><?php echo number_format($kpis['total_vendedores'] ?? 0, 0, ',', '.'); ?></p>
            </div>
        </div>
    </div>
        ...
        <form method="GET" class="filters-row">
            ...
            <div class="filter-group">
                ...
                <select name="f_vend">
                    ...
                    <?php foreach ($vendedores_filter as $vend): ?>
                        <option value="<?php echo htmlspecialchars($vend['usuario']); ?>" <?php echo ($f_vend == $vend['usuario']) ? 'selected' : ''; ?>>
                            <?php echo htmlspecialchars($vend['usuario']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
    ...
    <div class="tab-nav">
        <button class="tab-btn active" onclick="showTab('chart', event)">
            <i class="fas fa-chart-pie"></i> Análisis por Vendedor
        </button>
    ...
    <div id="chart" class="tab-content active">
        <div class="card table-card">
            <div class="table-header" style="border-bottom: 1px solid var(--border-light); padding-bottom: 15px; margin-bottom: 20px;">
                <h2 style="font-size: 1.1rem;"><i class="fas fa-chart-pie"></i> Distribución de Ventas por Vendedor</h2>
            </div>
            ...
            <div class="chart-section" style="padding: 20px;">
                ...
                <!-- Leyenda interactiva -->
                <div>
                    <h4 style="margin-top: 0; color: var(--primary); font-weight: 800;">Resumen por Vendedor</h4>
        ...
        <div class="card table-card" style="margin-top: 25px;">
            <div class="table-header" style="border-bottom: 1px solid var(--border-light); padding-bottom: 15px; margin-bottom: 20px;">
                <h2 style="font-size: 1.1rem;"><i class="fas fa-list-ul"></i> Consolidado por Televendedor</h2>
            </div>
            <div class="table-container">
                <table id="table-consolidado">
                    ...
                    <tbody>
                        <?php if (empty($resumen_vend)): ?>
                            <tr><td colspan="6" class="text-center" style="padding: 48px; opacity: 0.5;">No hay datos para mostrar.</td></tr>
                        <?php else: ?>
                            <?php $total_monto = array_sum(array_column($resumen_vend, 'monto')); ?>
                            <?php foreach ($resumen_vend as $r): ?>
                                <tr onclick="abrirModalArticulos('<?php echo $r['codvend']; ?>', '<?php echo htmlspecialchars(addslashes($r['vendedor'] ?? 'Sin Televendedor')); ?>')" 
                                    style="cursor: pointer;" title="Haga clic para ver artículos vendidos por este televendedor">
                                    <td style="font-weight: 700; color: var(--text-main);">
                                        <?php echo htmlspecialchars($r['vendedor'] ?? 'Sin Televendedor'); ?>
                                    </td>
                                    <td class="text-center"><?php echo $r['articulos']; ?></td>
                                    <td class="text-center"><?php echo $r['pedidos']; ?></td>
    ...
    <div id="table" class="tab-content">
        <div class="card table-card">
            <div class="t-header">
                <h2><i class="fas fa-list-ul"></i> Detalle de Artículos Vendidos</h2>
                <div style="font-size: 0.85rem; color: var(--text-muted); display: flex; align-items: center;">
                    ...
                    <button class="btn-neon btn-green" onclick="exportXls('table-articulos', 'Articulos_Vendidos')" style="margin-left: 15px; height: 32px; font-size: 0.7rem; padding: 0 12px; display:inline-flex; align-items: center;">
                ...
                <table id="table-articulos">
                    ...
                    <tbody>
                        <?php if (empty($articulos)): ?>
                            <tr><td colspan="9" class="text-center" style="padding: 48px; opacity: 0.5;">No se encontraron registros.</td></tr>
                        <?php else: ?>
                            <?php foreach ($articulos as $art): ?>
                                <tr>
                                    ...
                                    <td style="color: var(--primary); font-weight: 700; font-size: 0.8rem;">
                                        <?php echo htmlspecialchars($art['nombre_vendedor'] ?? 'Sin Televendedor'); ?>
                                    </td>
    ...
    <!-- MODAL: Detalle de Artículos por Vendedor -->
    <div id="modal-vendedor" class="modal-overlay">
        <div class="modal-box">
            <div class="modal-hd">
                <div>
                    <h3><i class="fas fa-shopping-basket" style="margin-right:10px;"></i>Artículos Vendidos</h3>
                    <div class="mref" id="modal-subtitle">Televendedor: —</div>
                </div>
...
async function loadChartData() {
    try {
        ...
        const url = `?ajax=vendedores&${params.toString()}`;
...
function renderDonutChart(data) {
    ...
    const labels = data.map(d => d.vendedor || 'Sin Televendedor');
...
function renderLegend(data) {
    ...
    data.forEach((item, idx) => {
        ...
        const nombreVendedor = item.vendedor || 'Sin Televendedor';
        html += `
            <div class="legend-item" onclick="abrirModalArticulos('${item.codigo}', '${nombreVendedor.replace(/'/g, "\\'")}')" style="display:flex; align-items:center; gap:10px; padding:10px; border-bottom:1px solid var(--border-light); cursor:pointer;">
                ...
                <div class="legend-text" style="flex:1;">
                    <strong style="font-size:0.85rem;">${nombreVendedor}</strong><br>
...
// Modal: Mostrar artículos por vendedor
async function abrirModalArticulos(codvend, nombre) {
    ...
    subtitle.innerText = `Televendedor: ${nombre} (${codvend})`;
    tbody.innerHTML = '';
    ...
    try {
        ...
        const response = await fetch(`?ajax=filtrar&codvend=${codvend}&f_ini=${f_ini}&f_fin=${f_fin}`);
        const data = await response.json();
        ...
        if (data.data.length === 0) {
            html = '<tr><td colspan="8" class="text-center" style="padding:40px; opacity:0.6;">No se encontraron artículos vendidos.</td></tr>';
        } else {
            data.data.forEach(art => {
                html += `
                    <tr>
                        ...
                        <td class="text-center" style="font-weight:700;">${parseFloat(art.cantidad_actual).toLocaleString('es-VE', {maximumFractionDigits: 2})}</td>
        ...
        tbody.innerHTML = `<tr><td colspan="8" class="text-center" style="color:var(--accent-red); padding:40px;">Error: ${err.message}</td></tr>`;
        loading.style.display = 'none';
        container.style.display = 'block';
    }
}
